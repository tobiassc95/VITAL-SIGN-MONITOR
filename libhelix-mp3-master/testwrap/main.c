...
static int FillReadBuffer(unsigned char *readBuf, unsigned char *readPtr, int bufSize, int bytesLeft, FILE *infile)
{
	...
	if (nRead < bufSize - bytesLeft) ///IF THE INPUT BUFFER DIDN'T FILL COMPLETELLY (WE READ THE WHOLE FILE)...
		memset(readBuf + bytesLeft + nRead, 0, bufSize - bytesLeft - nRead); ///THEN WE FILL THE REST OF THE INPUT BUFFER WITH 0s.
...
int main(int argc, char **argv)
{
	...
	do {
		...
		if (bytesLeft < 2*MAINBUF_SIZE && !eofReached) { ///IF WE ARE ABOUT TO FINISH READING THE INPUT BUFFER...
			nRead = FillReadBuffer(readBuf, readPtr, READBUF_SIZE, bytesLeft, infile); ///WE FILL THE INPUT BUFFER READING THE FILE.
			bytesLeft += nRead; ///UPDATE VARIABLES...
			readPtr = readBuf;
			if (nRead == 0)
				eofReached = 1;
		}
		...
		if (offset < 0) { ///IF THERE ISN'T ANT SYNC WORD, THEN EOF (WE READ THE WHOLE FILE).
			outOfData = 1;
			break;
		}
		readPtr += offset; ///UPDATE VARIABLES...
		bytesLeft -= offset;
