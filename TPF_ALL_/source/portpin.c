...
void PORTC_IRQHandler(void) {
	...
	for (i = 0; i < N_ISRPINS; i++) {
		if (PIN2PORT(isrpin[i].pin) == PORT_C) {
			if(port[PORT_C]->PCR[PIN2NUM(isrpin[i].pin)] & PORT_PCR_ISF_MASK) { //nos fijamos si es el pin correspondiente a la interrupcion
				port[PORT_C]->PCR[PIN2NUM(isrpin[i].pin)] |= PORT_PCR_ISF_MASK; //ponemos en cero el interrupt flag. w1c.
				isrpin[i].funcptr();
			}
		}
	}
