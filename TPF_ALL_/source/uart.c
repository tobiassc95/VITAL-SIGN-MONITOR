...
//UART0
#define PTA2 PORTNUM2PIN(PORT_A, 2) //TX
#define PTA1 PORTNUM2PIN(PORT_A, 1) //RX
//UART1
...
void UARTconfigure (uint8_t uart_, uint16_t baudrate, uint8_t parity) {
	...
	switch (uart_) {
	case UART_0:
		PINconfigure(PTA2, PIN_MUX2, PIN_IRQ_DISABLE);
		PINconfigure(PTA1, PIN_MUX2, PIN_IRQ_DISABLE);
		break;
	case UART_1:
...
uint16_t UARTreadMSG(uint8_t uart_, char* msg, uint16_t Nbytes) { //when we recieve, we read.
	uint16_t n = 0; //counts the amount of bytes that are read.

	if (Nbytes > rxfifo.Nbytes)
		return 0;

	disableUARTinterrupt(uart_);
	while (Nbytes--) {
		if (rxfifo.Nbytes == 0) //If there is no data to read...
			break;
	...
	enableUARTinterrupt(uart_);
	return n; //return the msg's length in bytes.
}
...
__ISR__ UART0_RX_TX_IRQHandler (void) {
	...
	if (temp & UART_S1_TDRE_MASK) { //si transmite (TX)
		if (txfifo.Nbytes <= 1) //stop generating interrupts when we are running out of bytes.
...
__ISR__ UART1_RX_TX_IRQHandler (void) {
	...
	temp = UART1->S1;
	if(temp & UART_S1_RDRF_MASK) { //si recive (RX)
		rxfifo.buff[rxfifo.last] = UART1->D;
		if(++rxfifo.last == BUFFER_LEN)
			rxfifo.last = 0;
		rxfifo.Nbytes++;
	}
	...
	if (temp & UART_S1_TDRE_MASK) { //si transmite (TX)
		...
		if (txfifo.Nbytes) {
			txfifo.Nbytes--;
			UART1->D = (uint8_t) txfifo.buff[txfifo.first]; //Transmit
			if(++txfifo.first == BUFFER_LEN)
				txfifo.first = 0;
		}
	}
...
__ISR__ UART3_RX_TX_IRQHandler (void) {
	...
	temp = UART3->S1;
	if(temp & UART_S1_RDRF_MASK) { //si recive (RX)
		rxfifo.buff[rxfifo.last] = UART3->D;
		if(++rxfifo.last == BUFFER_LEN)
			rxfifo.last = 0;
		rxfifo.Nbytes++;
	}
	...
	if (temp & UART_S1_TDRE_MASK) { //si transmite (TX)
		...
		if (txfifo.Nbytes) {
			txfifo.Nbytes--;
			UART3->D = (uint8_t) txfifo.buff[txfifo.first]; //Transmit
			if(++txfifo.first == BUFFER_LEN)
				txfifo.first = 0;
		}
	}
